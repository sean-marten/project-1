    ...
    function getMovieData(personID) {
      ...
      $.get(tmdbMovies, function (response) {
        ...
        $.each(sortRating, function (index) {
          ...
          const cardBody = $('<div>').addClass('card-body deep-blue-gradient text-dark')
          ...
          const cardText = $('<p>').addClass('card-text text-dark').text(sortRating[index].overview)
          const cardRating = $('<p>').addClass('card-text text-dark').text('Rating: ' + sortRating[index].vote_average + '/10')
